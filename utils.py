...
class DelightingDataset(Dataset):

    def __init__(self, root_dir, img_size=512, is_train=True):
        self.root_dir = root_dir
        self.dirs = glob.glob(os.path.join(self.root_dir, 'light_image', '*.jpg'))
        self.img_size = img_size
        if is_train:
            self.augm = DataAugmentation(
                img_size=self.img_size,
    ...
    def __getitem__(self, idx):
        ...
        suff = '_' + this_dir.split('_')[-1]
        this_gt_dir = this_dir.replace('light_image', 'ground_truth').replace(suff, '.jpg')
        gt1 = cv2.imread(this_gt_dir, cv2.IMREAD_COLOR)
        gt1 = cv2.cvtColor(gt1, cv2.COLOR_BGR2RGB)
...
def get_loaders(args):
    ...
    if args.dataset == 'dogs':
        ...
        training_set = DelightingDataset(
            root_dir=r'./datasets/Light/train', img_size=512, is_train=True)
        val_set = DelightingDataset(
            root_dir=r'./datasets/Light/val', img_size=512, is_train=False)
...
def cpt_rgb_ssim(img, img_gt):
    ...
    for i in range(3):
        ...
        ssim = structural_similarity(tmp, tmp_gt)
        SSIM = SSIM + ssim
    return SSIM / 3.0
