...
class GRGAN():
    ...
    def __init__(self, args, dataloaders):
        ...
        self.net_G = cycnet.define_G(
            input_nc=3, output_nc=6, ngf=args.ngf, netG=args.net_G, use_dropout=False, norm='none',
            is_attention=args.is_attention, attention_num=args.attention_num, attention_location=args.attention_location).to(device)
            # fpn norm = instance
    ...
    def train_models(self):
        ...
        for self.epoch_id in range(self.epoch_to_start, self.max_num_epochs):
            ...
            for self.batch_id, batch in enumerate(self.dataloaders['train'], 0):
                ...
                # # TensorBoard: G_loss
                # writer.add_scalar("batch_G/D_loss", self.G_loss, self.batch_id)
                # # TensorBoard: D_loss
                # writer.add_scalar("batch_G/D_loss", self.D_loss, self.batch_id)
                # # TensorBoard: acc
                # writer.add_scalar("batch accuracy: psnr", np.mean(self.running_acc), self.batch_id)



            # # TensorBoard: G_loss
            # writer.add_scalar("epoch_G/D_loss", self.G_loss, self.epoch_id)
            # # TensorBoard: D_loss
            # writer.add_scalar("epoch_G/D_loss", self.D_loss, self.epoch_id)
            # # TensorBoard: acc
            # writer.add_scalar("epoch accuracy: psnr", self.epoch_acc, self.epoch_id)
