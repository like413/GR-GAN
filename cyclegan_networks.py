...
from modules.fpn_mobilenet import FPNMobileNet
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[],
    ...
    Parameters:
        ...
        netG (str) -- the architecture's name: resnet_9blocks | resnet_6blocks | unet_256 | unet_128 | fpn_mobilenet | fpn_inception
        ...
        attention_location: 0 before 1x1 conv Lateral connections, 1 behind 1x1 conv Lateral connections
    ...
    # add fpn_mobilenet module
    elif netG == 'fpn_mobilenet':
        # model_g = FPNMobileNet(norm_layer=get_norm_layer(norm_type=instance]))
        net = FPNMobileNet(norm_layer=norm_layer, output_ch=output_nc,
                           is_attention=is_attention, attention_num=attention_num, attention_location=attention_location)
    # add fpn_inception module
    elif netG == 'fpn_inception':
        # model_g = FPNInception(norm_layer=get_norm_layer(norm_type=model_config['norm_layer']))
        net = FPNInception(norm_layer=norm_layer, output_ch=output_nc,
                           is_attention=is_attention, attention_num=attention_num, attention_location=attention_location)
